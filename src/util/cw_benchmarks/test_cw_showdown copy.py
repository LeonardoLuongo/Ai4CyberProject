...
def main():
    ...
    # 1C. CUSTOM C&W (Binary Steps)
    print("-> [3. CUSTOM BINARY] Esecuzione attacco BinarySteps...")
    ...
    print(f"| Metrica         | ART Originale | Custom ART-Match | Custom BinarySteps |")
    print(f"|-----------------|---------------|------------------|--------------------|")
    print(f"| Tempo (sec)     | {art_untarg_time:13.2f} | {full_untarg_time:16.2f} | {cust_untarg_time:18.2f} |")
    print(f"| Success Rate    | {art_untarg_succ:12.1f}% | {full_untarg_succ:15.1f}% | {cust_untarg_succ:17.1f}% |")
    print(f"| L_inf Max       | {art_untarg_max:13.4f} | {full_untarg_max:16.4f} | {cust_untarg_max:18.4f} |")
    print(f"| L_inf Mean      | {art_untarg_mean:13.4f} | {full_untarg_mean:16.4f} | {cust_untarg_mean:18.4f} |")
    ...
    if art_targeted_cache.exists():
        ...
        art_cw_t = CarliniLInfMethod(classifier=classifier, targeted=True, max_iter=100, learning_rate=0.01, batch_size=10, verbose=False)
    ...
    start = time.time()
    x_adv_cust_targ = custom_cw_targ.forward(x_clean, y_target_llc)
    ...
    print(f"| Metrica         | ART Originale | Custom ART-Match | Custom BinarySteps |")
    print(f"|-----------------|---------------|------------------|--------------------|")
    print(f"| Tempo (sec)     | {art_targ_time:13.2f} | {full_targ_time:16.2f} | {cust_targ_time:18.2f} |")
    print(f"| Success Rate    | {art_targ_succ:12.1f}% | {full_targ_succ:15.1f}% | {cust_targ_succ:17.1f}% |")
    print(f"| L_inf Max       | {art_targ_max:13.4f} | {full_targ_max:16.4f} | {cust_targ_max:18.4f} |")
    print(f"| L_inf Mean      | {art_targ_mean:13.4f} | {full_targ_mean:16.4f} | {cust_targ_mean:18.4f} |")
