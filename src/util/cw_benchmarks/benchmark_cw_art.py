...
def plot_benchmark_v4(clean_np, adv_es_np, adv_full_np, adv_auto_np, adv_art_np, out_path):
    ...
    fig.suptitle('Global Benchmark C&W: Custom vs AutoTemp vs ART', fontsize=16)
...
def main():
    print("======================================================")
    print(" GLOBAL BENCHMARK C&W (ES vs FULL vs AUTOTEMP vs ART) ")
    print("======================================================\n")
    ...
    print(f"[OK] Immagine Trovata (True ID: {true_facenet_id}). Target ID (Global): {target_id_global}\n")

    wrapped_model_spliced.freeze_target_classes(sample_img_01)
    target_idx_local = int(torch.where(wrapped_model_spliced.active_indices[0] == target_id_global)[0].item())
    local_y = torch.tensor([target_idx_local], dtype=torch.long, device=device)

    # --- TEST 1: CUSTOM EARLY STOP ---
    print("-> Esecuzione 1: Custom C&W (Early Stop)...")
    attack_es = PyTorchCarliniLInf_EarlyStop(model=wrapped_model_spliced, targeted=True, max_iter=50)
    t0 = time.time()
    ...
    succ_es = (int(torch.argmax(wrapped_model_full(adv_es), dim=1).cpu()[0]) == target_id_global)

    # --- TEST 2: CUSTOM FULL OPTIMIZED ---
    print("-> Esecuzione 2: Custom C&W (True SOTA Binary Search)...")
    attack_opt = PyTorchCarliniLInf_BinarySteps(model=wrapped_model_spliced, targeted=True, max_iter=50)
    t0 = time.time()
    ...
    succ_full = (int(torch.argmax(wrapped_model_full(adv_full), dim=1).cpu()[0]) == target_id_global)
    
    # --- TEST 3: CUSTOM AUTO TEMP SCALING ---
    print("-> Esecuzione 3: Custom C&W (Auto Temp Scaling + Binary Search)...")
    attack_auto = PyTorchCarliniLInf_AutoTemp(model=wrapped_model_spliced, targeted=True, max_iter=50)
    t0 = time.time()
    ...
    succ_auto = (int(torch.argmax(wrapped_model_full(adv_auto), dim=1).cpu()[0]) == target_id_global)
    ...
    y_target_np = np.array([target_id_global])
    ...
    succ_art = (int(torch.argmax(wrapped_model_full(adv_art_tensor), dim=1).cpu()[0]) == target_id_global)

    # --- RISULTATI ---
    print("\n======================================================")
    print(" SOMMARIO DEI RISULTATI GLOBALI ")
    print("======================================================")
    print(f"Metodo       | Tempo (s) | L_inf Dist | Successo")
    print(f"Custom (ES)  | {time_es:9.2f} | {linf_es:10.4f} | {succ_es}")
    print(f"Custom (Full)| {time_full:9.2f} | {linf_full:10.4f} | {succ_full}")
    print(f"Custom (Auto)| {time_auto:9.2f} | {linf_auto:10.4f} | {succ_auto}")
    print(f"ART (SOTA)   | {time_art:9.2f} | {linf_art:10.4f} | {succ_art}")
    print("======================================================")

    plot_path = str(out_dir / "global_benchmark_cw_v4.png")
