...
def create_strategy_plots_and_metrics(
    ...
    targeted_curve = {f"PGD Targeted Transfer ({strategy})": []}
    ...
    for eps in epsilons:
        ...
        targeted_curve[f"PGD Targeted Transfer ({strategy})"].append(targeted_asr)
        ...
        if logger is not None:
            logger.log_attack_metrics(
                tester="Andrea",
                attack_type="PGD Targeted Transfer NN1->NN2",
                strategy=strategy,
                epsilon=eps,
                defense_type="None",
    ...
    plot_target_confidence_growth(
        epsilons,
        target_confidence_data,
        f"PGD Targeted Transfer ({strategy})",
        True,
...
def main() -> None:
    print("====================================================================")
    print(" METRICHE & PLOT: PGD TARGETED TRANSFERABILITY NN1 -> NN2 SENET50  ")
    print("====================================================================\n")
    ...
    for strategy in STRATEGIES:
        ...
        evaluated_csv = output_dir / f"pgd_targeted_transfer_evaluated_{strategy}.csv"
        ...
        pd.DataFrame(metric_rows).to_csv(
            output_dir / f"pgd_targeted_transfer_metrics_{strategy}.csv",
            index=False,
        )
    ...
    if all_metric_rows:
        pd.DataFrame(all_metric_rows).to_csv(
            plots_root / "pgd_targeted_transfer_metrics_all_strategies.csv",
            index=False,
        )

    print("\n[OK] Valutazione targeted PGD NN1 -> SENet50-FT completata.")
