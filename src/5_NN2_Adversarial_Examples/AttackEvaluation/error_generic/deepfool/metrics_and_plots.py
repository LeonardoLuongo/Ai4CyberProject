...
def load_caffe_weights(model, weights_path):
    """Carica i pesi Caffe convertiti per SENet50-FT."""
...
def preprocess_for_senet(img_bgr_01):
    """Resize 160x160 -> 224x224 e preprocessing Caffe per SENet50-FT."""
...
def main():
    print("======================================================")
    print(" METRICHE & PLOT: DEEPFOOL TRANSFERABILITY NN1 -> NN2 ")
    print("======================================================\n")
    ...
    output_eval_dir = base_dir / "plots" / "5_NN2_Adversarial_Examples" / "NN2" / "error_generic" / "deepfool"
    ...
    print(f"-> Caricamento NN2 SENet50-FT su {device}...")
    ...
    with torch.inference_mode():
        for start_idx in tqdm(
            ...
            for _, row in batch_df.iterrows():
                ...
                x_clean_list.append(preprocess_for_senet(clean_bgr))
                x_adv_list.append(preprocess_for_senet(adv_bgr))
    ...
    asr_dict = {"DeepFool Transfer NN1->NN2": []}
    confidence_data = []
    previous_eps = 0.0
    ...
    for eps in epsilons:
        ...
        try:
            logger.log_attack_metrics(
                tester="Andrea",
                attack_type="DeepFool Transfer NN1->NN2",
                strategy="Untargeted",
                epsilon=eps,
                defense_type="None",
        ...
        asr_dict["DeepFool Transfer NN1->NN2"].append(robust_accuracy)
    ...
    for eps in epsilons:
        ...
        if not suitable_samples.empty:
            ...
            plot_adversarial_showcase(
                ...
                f"NN2 Pred: ID {sample['adv_pred_class']}",
                True,
    ...
    print("\n[OK] Pipeline DeepFool Transferability NN1 -> NN2 conclusa!")
