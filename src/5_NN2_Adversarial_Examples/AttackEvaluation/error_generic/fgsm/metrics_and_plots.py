...
# DIMENSIONE AGGIORNATA PER SENET50
IMAGE_SIZE = 224
...
def main():
    print("======================================================")
    print(" METRICHE & PLOT: FGSM ERROR-GENERIC (NN2 - SENet50)  ")
    print("======================================================\n")
    ...
    # =========================================================
    # PATH AGGIORNATI PER PUNTARI ALLA CARTELLA DI NN2
    attacks_dir = base_dir / "dataset" / "attacks" / "NN2" / "error_generic" / "fgsm"
    output_eval_dir = base_dir / "plots" / "3_Adversarial_Examples" / "NN2" / "error_generic" / "fgsm"
    ...
    if weights_path.exists():
        ...
        raise FileNotFoundError(f"File dei pesi non trovato in: {weights_path}")
        ...
        for eps in epsilons:
            ...
            for i in tqdm(range(0, len(df_eps), batch_size), desc=f"Inferenza eps={eps:.3f}"):
                ...
                for _, row in batch_df.iterrows():
                    ...
                    # load_rgb_image ridimensiona in automatico a 224x224
    ...
    evaluated_csv_path = output_eval_dir / "fgsm_untargeted_evaluated_nn2.csv"
    ...
    for eps in epsilons:
        ...
        asr_dict["FGSM Untargeted (NN2)"].append(robust_accuracy)
        ...
        if hasattr(logger, 'log_attack_metrics'):
            logger.log_attack_metrics(
                tester="Andrea",
                attack_type="NN2 FGSM Error-Generic",
                strategy="Untargeted",
                epsilon=eps,
                defense_type="None",
                ...
                notes="Valutazione Untargeted su SENet50 (224x224)"
            )
        else:
            logger.log_biometric_metrics(
                tester="Andrea", 
                phase="Error-Generic Evaluation NN2",
                attack_type="FGSM NN2",
                epsilon=eps,
                defense_type="None",
    ...
    plot_security_evaluation_curves(epsilons, asr_dict, "NN2 (SENet50)", True, str(output_eval_dir / "robust_accuracy_curve_nn2.png"))
    plot_confidence_degradation(epsilons, confidence_data, "FGSM Untargeted (NN2)", True, str(output_eval_dir / "confidence_degradation_nn2.png"))
    ...
    print("\n[OK] Pipeline di Evaluation Error-Generic (NN2) conclusa con successo!")
