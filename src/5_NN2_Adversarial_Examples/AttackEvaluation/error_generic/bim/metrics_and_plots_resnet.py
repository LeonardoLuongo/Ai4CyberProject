...
def load_caffe_weights(model, weights_path: Path) -> None:
    ...
    for name, param in weights.items():
        if name in own_state:
            ...
            print(f"[WARNING] Chiave inattesa nei pesi ResNet: {name}")
...
def preprocess_rgb_for_resnet(image_rgb: np.ndarray) -> np.ndarray:
    """ResNet VGGFace2: RGB float32 [0, 1] -> BGR [0, 255] - MEAN_BGR -> CHW."""
...
def main():
    print("================================================================")
    print(" METRICHE & PLOT: BIM TRANSFERABILITY NN1 -> RESNET50-SCRATCH  ")
    print("================================================================\n")
    ...
    if not tracker_files:
        raise FileNotFoundError(
            ...
            "Sono richiesti i tracker BIM gia generati contro NN1."
        )
    ...
    print("-> Resize float32 a 224x224 e preprocessing Caffe per ResNet50-Scratch.\n")
    ...
    if not weights_path.exists():
        raise FileNotFoundError(f"Pesi ResNet50-Scratch mancanti: {weights_path}")

    load_caffe_weights(nn2, weights_path)
    nn2 = nn2.eval().to(device)
    print("-> Pesi ResNet50-Scratch caricati correttamente.")
        ...
        for eps in epsilons:
            ...
            for start in tqdm(range(0, len(df_eps), batch_size), desc=f"Inferenza eps={eps:.3f}"):
                ...
                for _, row in batch_df.iterrows():
                    ...
                    clean_batch.append(preprocess_rgb_for_resnet(clean_rgb))
                    adv_batch.append(preprocess_rgb_for_resnet(adv_rgb))
    ...
    robust_accuracy_curve = {"BIM Transfer NN1->ResNet50-Scratch": []}
    confidence_data = []
    transfer_asr_values = []
    metric_rows = []
    ...
    for eps in epsilons:
        ...
        robust_accuracy_curve["BIM Transfer NN1->ResNet50-Scratch"].append(robust_accuracy)
        ...
        if logger is not None:
            logger.log_attack_metrics(
                tester="Andrea",
                attack_type="BIM Transfer NN1->ResNet50-Scratch",
    ...
    plot_security_evaluation_curves(
        ...
        "Transferability NN1 -> NN2 (ResNet50-Scratch)",
        True,
    ...
    plot_confidence_degradation(
        epsilons,
        confidence_data,
        "BIM Transfer NN1->ResNet50-Scratch",
        True,
    ...
    plot_transferability_matrix(
        transfer_matrix,
        source_models=["NN1"],
        target_models=["NN2 ResNet50-Scratch"],
    ...
    print("\n[OK] Valutazione transferability BIM NN1 -> ResNet50-Scratch completata.")
