...
# Assicurati che l'import punti correttamente alla tua utils
...
def main() -> int:
    ...
    if output_dir is None:
        ...
        output_dir = default_output_dir(base_dir, "fgsm", eps)
    ...
    for batch_index, batch_samples in enumerate(batched(samples, batch_size), start=1):
        ...
        for sample, original, adversarial in zip(batch_samples, x, x_adv):
            ...
            # STRUTTURA DEL MANIFEST AGGIORNATA
            rows.append(
                {
                    "attack_type": "fgsm",  # Cambiato da fgsm_error_generic a fgsm come da tua richiesta
                    "eps": eps,
                    "targeted": False,  # Untargeted attack
                    "target_strategy": -1, # Nessuna strategia per untargeted
                    "target_class": -1,  # Nessuna classe bersaglio
