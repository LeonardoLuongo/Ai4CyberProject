...
# Importiamo la tua classe C&W ottimizzata SOTA e il mapper
...
def main():
    print("======================================================")
    print(" SCOUTING HYPER-PARAMS: C&W UNTARGETED (Pure-GPU)     ")
    print("======================================================\n")
    ...
    # Estimator Dummy per l'interfaccia della tua classe
    ...
    x_clean_tensor = torch.stack(valid_x).to(device)
    y_clean_tensor = torch.tensor(valid_y, dtype=torch.long, device=device)
    x_clean_np = x_clean_tensor.cpu().numpy()
    total_samples = len(valid_x)
    print(f"-> Immagini valide raccolte: {total_samples}")

    # --- FASE 2: GRID SEARCH AVVERSARIA ---
    print("\n[FASE 2] Avvio C&W Grid Search (Untargeted, Full-Network)...\n")
    
    if torch.cuda.is_available(): torch.cuda.empty_cache()
    
    with open(txt_log_path, 'w') as f:
        f.write(f"REPORT SCOUTING CARLINI & WAGNER UNTARGETED (PURE-GPU)\n")
    ...
    for lr in learning_rates:
        ...
        for steps in max_iters_list:
            ...
            # Usiamo la tua classe ottimizzata (Senza TopK)
            ...
            x_adv_np = attack.generate(x=x_clean_np)
            ...
            with torch.no_grad():
                ...
                success_np = (adv_preds != y_clean_tensor).cpu().numpy()
            ...
            plot_data[lr]['acc'].append(robust_accuracy * 100)
            plot_data[lr]['linf'].append(l_mean)
            ...
            stats_str = (
                ...
                f"-> Risultato: Robust Accuracy = {robust_accuracy*100:.2f}%\n"
            ...
            if robust_accuracy == 0.0:
                msg = f"   [!] Accuracy crollata a 0.00%. Salto max_iter successivi per questo LR per risparmiare tempo.\n"
                print(msg)
                with open(txt_log_path, 'a') as f:
                    f.write(msg)
    ...
    for i, lr in enumerate(unique_lrs):
        ...
        if is_stopped:
            ...
            ax1.annotate("0.0%", (last_iter, last_acc), textcoords="offset points", xytext=(0, 15), ha='center', color=color, fontweight='bold', fontsize=10)
    ...
    ax2.set_title(r"Mean $L_\infty$ Perturbation vs. Max Iterations", fontsize=14, fontweight='bold')
    ax2.set_xlabel("Max Iterations (steps)", fontsize=12)
    ax2.set_ylabel(r"Mean $L_\infty$ Norm", fontsize=12)
    ...
    fig.legend(global_handles, global_labels, loc='upper center', bbox_to_anchor=(0.5, 0.90), ncol=num_cols, fontsize=11, frameon=True)
    
    plt.suptitle("Carlini & Wagner Untargeted: Hyperparameter Tuning", fontsize=18, y=0.98)
